...
export default function ClientContractsPage() {
  ...
  const loadContracts = async () => {
    ...
    try {
      ...
      // Fetch contracts for this client with vendor and event details
      const { data: contractsData, error: contractsError } = await supabase
        .from('contracts')
        .select(`
          *,
          event:events(id, event_type, date, city),
          vendor:vendors(id, company_name, user:users(name, email))
        `)
  ...
  const getSignatureStatus = (contract: Contract) => {
    ...
    if (clientSigned && vendorSigned) {
      return { text: 'Fully Signed', color: 'text-green-400' };
    } else if (clientSigned) {
      return { text: 'Waiting for Vendor', color: 'text-orange-400' };
    } else {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
            ...
            <p className="text-3xl font-bold text-orange-400">
              {contracts.filter(c => c.contract_status === 'PENDING').length}
            </p>
          </div>
          ...
          <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700/50 rounded-xl p-6">
            ...
            <p className="text-2xl font-bold text-green-400">
              ₹{(contracts.reduce((sum, c) => sum + (c.total_amount || 0), 0) / 100000).toFixed(2)}L
            </p>
          </div>
        </div>
            ...
            {filteredContracts.map((contract) => {
              ...
              return (
                <div
                  ...
                  onClick={() => router.push(`/dashboard/client/contracts/${contract.id}`)}
                >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                          <div className="flex items-center space-x-2 text-slate-300">
                            ...
                            <span className="text-sm">{contract.vendor?.company_name || 'Vendor'}</span>
                          </div>
